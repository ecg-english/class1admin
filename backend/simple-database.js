...
async function init() {
  console.log('=== シンプルSQLiteデータベース初期化開始 ===');
...
async function getStatus() {
  return new Promise((resolve) => {
    try {
      const status = {
        databaseExists: fs.existsSync('/tmp/class1admin.db'),
        databaseSize: fs.existsSync('/tmp/class1admin.db') ? fs.statSync('/tmp/class1admin.db').size : 0,
